...
from campa.tl import ModelEnumTorch, LossEnumTorch #, LossEnum, ModelEnum
...
def _prepare_exp_split(exp) -> None:
    """
...
class TorchExperiment:
    ...
    @staticmethod
    def get_experiments_from_config(config_fname: str, exp_names: Optional[Iterable[str]] = None):
        """
        Initialise and return experiments from configs in config file.
        ...
        for exp_config in config.variable_config:
            cur_config = merged_config(config.base_config, exp_config)
            if exp_names is None or cur_config["experiment"]["name"] in exp_names:
                exps.append(TorchExperiment(cur_config))
        return exps
    ...
    def get_experiments_from_dir(
        ...
        for exp_name in next(os.walk(os.path.join(campa_config.EXPERIMENT_DIR, exp_dir)))[1]:
            config_fname = os.path.join(campa_config.EXPERIMENT_DIR, exp_dir, exp_name, "config.json")
            if os.path.exists(config_fname) and ((exp_names is None) or (exp_name in exp_names)):
                exp = TorchExperiment.from_dir(os.path.join(exp_dir, exp_name))
                if not only_trainable or exp.is_trainable:
                    exps.append(exp)
        return exps
        

def run_torch_experiments(exps, mode: str = "all") -> None:
    """
    Execute experiments.
